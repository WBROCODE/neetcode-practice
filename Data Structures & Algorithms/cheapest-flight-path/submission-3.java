class Solution {
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
        ...
        while(!pq.isEmpty()){
            ...
            if(curCost > costs[curNode][remainK]) continue;
            ...
            for(int[]next : graph[curNode]){
                ...
                int nextCost = next[1];
                int nextK = remainK - 1;
                if(nextK >= 0 && curCost + nextCost < costs[nextNode][nextK] ){
                    costs[nextNode][nextK] = curCost + nextCost;
                    pq.offer(new int[]{nextNode, costs[nextNode][nextK], nextK});
                }
            }
        }
